...
int hashSymbolEntry(HASHSYMBOL h, SYMBOLTABLE t, SYMBOLENTRY e, FIELD f, char* function, char* id, char* fieldid, int size){
...
int lookupEntry(HASHSYMBOL h, char *function, char *id, char *fieldid, int size) {
...
void functionSymbolTable(NODE_AstTree function, HASHSYMBOL h, int hash_size) {
	...
	for(; input_par!=NULL; input_par=input_par->sibling) {
		...
		if(input_par->tokens->tk->token==TK_INT) {
			...
			TOKENINFO t = input_par->tokens->next->tk;
			FIELD f = fieldinit(REAL, t->val->valF,t->lexeme);
	...
	for(; declarations!=NULL; declarations=declarations->sibling) {
		if(decls->tokens->next->next->tk->token==NULL) {
			if(output_par->tokens->tk->token==TK_INT) {
...
HASHSYMBOL populateSymbolTable(NODE_AstTree ast) {
	...
	for(; record_temp!=NULL; record_temp=record_temp->sibling) {
		...
		for(; records!=NULL; records=records->sibling) {
			...
			hval = duplicateLookup(h, record_table, s, NULL, "record", s->id, NULL, hash_size, "Duplicate record\n");
	...
	while(f_temp!=NULL) {
		functionSymbolTable(f_temp, h, hash_size);
